...
import styles from './PickSingleEliminationBracketRegion.module.css';
import PickBracketMatchup from './PickBracketMatchup';
import PickBracketMatchup_INACTIVE from './PickBracketMatchup_INACTIVE';
...
const PickSingleEliminationBracketRegion = ({ data, regionNum, sendBracketUp, currentBracketBuild }) => {
    ...
    // Reverse every other region to make them all converge in the center
    const forwardBracket = Object.keys(bracket);
    const reverseBracket = Object.keys(bracket).slice().reverse();

    const forwardCells = Object.keys(bracket);
    const reverseCells = Object.keys(bracket).slice().reverse();

    let chosenBracket = regionNum % 2 == 0 ? reverseBracket : forwardBracket;
                        ...
                        <div key={`${round}_${i}`} className={styles.column}>
                            {
                                Object.keys(bracket[round]).map((matchup, j) => {
                                    ...
                                    return (
                                        <div key={`inline_connector_${j}`} className={(round === '1' && regionNum % 2 == 0) ? styles.inlineConnecterRight : styles.inlineConnecter}>
                                            ...
                                            <div key={`cell_${j}`} className={styles.cell}>
                                                {
                                                    bracket[round][matchup].team1 === null && bracket[round][matchup].team2 === null
                                                        ?
                                                        <PickBracketMatchup_INACTIVE key={`inactive_cell_${j}`} oneTeam={false} />
                                                        :
                                                        <PickBracketMatchup key={`cell_${bracket[round][matchup].team1}_${bracket[round][matchup].team2}`} region={regionNum} seedingEnabled={seedingOn} seed1={bracket[round][matchup].team1} seed2={bracket[round][matchup].team2} sendBracketUp={sendBracketUp} currentBracketBuild={currentBracketBuild} />
...
export default PickSingleEliminationBracketRegion